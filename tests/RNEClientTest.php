...
class RNEClientTest extends TestCase
{
    ...
    public function testAuthenticateWithBadRequest(): void
    {
        $fakeResponse = '{"code":"400","webserviceCode":"","errorCode":"bad_request","message":"","type":"bad_request"}';
        $mockHandler = new MockHandler([new Response(400, [], $fakeResponse)]);
        $handlerStack = HandlerStack::create($mockHandler);
        $mockedClient = new Client(['handler' => $handlerStack]);
    ...
    public function testAuthenticateNeeded(): void
    {
        ...
        $this->RNEClient = new SearchCompanies(null, $mockedClient);

        $this->expectException(\Exception::class);
        $this->expectExceptionMessage("You need to authenticate first.");

        $this->RNEClient->searchByMultipleSiren(['889924320', '894419969']);
    }
    ...
    public function testAuthenticateWithInternalErrorException(): void
    {
        $fakeResponse = '{
            "code": "500",
    ...
    public function testAuthenticateWithUnknowErrorException(): void
    {
        $fakeResponse = '{
            "code": "wtf",
        ...
        $mockHandler = new MockHandler([new Response(501, [], $fakeResponse)]);
        $handlerStack = HandlerStack::create($mockHandler);
        $mockedClient = new Client(['handler' => $handlerStack]);
